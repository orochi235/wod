...
what a wedge has to hold. `turnFraction` in `studio/wedge.ts` already prints
them and moves somewhere both callers reach.
...
`src/editor/bands.ts`: `bandsOf(breakpoints, ceiling)`, `splitAt`, `removeAt`,
and the log scale both ways. That file carries the unit tests. `BandTrack`'s own
test covers selection and one drag round-trip, nothing more.

## Split and merge

There is no "add breakpoint" button. Clicking a band splits it, and the upper
half inherits the split band's slice with its params deep-copied, so a split
resolves every width exactly as it did a moment before. Removing a stop merges
its band down into the neighbour below.
...
`BreakpointPanel` keeps the track at the top, holds the selected index, and
...
so it cannot be selected, split from the left, or removed. Clicking it selects
nothing.
